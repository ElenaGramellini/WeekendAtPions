...
Figures \ref{fig:CorrectionsBeam} show $C^{\pi MC}_{Interacting} (E_{i})$ and $C^{\pi MC}_{Incident} (E_{i})$ as a function of momentum for the 60A runs and their systematic uncertainty. 
...
\begin{equation}
N^{\text{$\pi$ Reco Data}}_{\text{Interacting}} (E_{i}) =  N^{TOT Data}_{Interacting} (E_{i}) - B^{Data}_{Interacting} (E_i)  =  C^{\pi MC}_{Interacting} (E_{i}) N^{TOT Data}_{Interacting} (E_{i}).
\end{equation}




%Figure \ref{fig:trueScatteringAngle} shows the distribution of the simulated scattering angles for elastic scattering as predicted by Geant4. 
%In  \ref{sec:angleRes}) Since the angle resolution in data is estimated to be $\bar\alpha_{Data} = (5.0 \pm 4.5) \text{ deg}$  
