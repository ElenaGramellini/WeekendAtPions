...
Interaction angles smaller than the angle resolution are indistinguishable for the reconstruction. Therefore, our ability to measure the cross section is limited to interaction angles greater than 5.0 deg. A small $0.5 \text{ deg}$ systematic shift between the mean of the data and MC angular resolution is present, which we account for in the context of the MC efficiency correction to the cross section, as presented in \ref{sec:angSys}.
More accurate studies of the angular resolution as a function of the kinetic energy and track lenght, albeit interesting, are left for an improvement of the analysis.
...
$\bar\alpha_{MC} = (4.5 \pm 3.9) \text{ deg}$, respectively. Interaction angles smaller than the angular resolution are indistinguishable for the reconstruction. Thus, we claim we are able to  measure the cross section for interaction angles greater than 5.0 deg.

Therefore, we select events which have an interaction angle greater than a given $\alpha_res$ in order to construct the true interacting and incident histograms and calculate the efficiency correction. 
